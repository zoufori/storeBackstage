    ...
    <div class="Hui-article">
        <article class="cl pd-20">
            <div class="text-c"> 日期范围：
                ...
                <form action="${pageContext.request.contextPath}/admin/findByUsername.do" method="post">
                    <input type="text" class="input-text" style="width:250px" placeholder="输入管理员名称" id="" name="text">
            ...
            <div class="cl pd-5 bg-1 bk-gray mt-20">
                <span class="l"> <a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i
                        class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;"
                                                                      onclick="admin_add('添加管理员','${pageContext.request.contextPath}/admin/addAdmin.do','800','500')"
            ...
            <table class="table table-border table-bordered table-bg">
                ...
                <tr class="text-c">
                    <th width="40">ID${cookie.jksjdnvvkdjfnvsdf}</th>
                    <th width="150">登录名</th>
                    <th width="90">密码</th>
                    <th width="150">角色</th>
                ...
                <tbody>
                <c:forEach var="list" items="${adminList.list}">
                    <tr class="text-c">
                        <td>${list.id}</td>
                        <td>${list.username}</td>
                        <td>${list.password}</td>
                        ...
                        <td class="td-manage"><a style="text-decoration:none" onClick="admin_start(this,${list.id})"
                                                 href="javascript:;" title="停用或启用"><i class="Hui-iconfont">&#xe631;</i></a>
                            <a title="编辑" href="javascript:;"
                               onclick="admin_edit('管理员编辑','${pageContext.request.contextPath}/admin/addAdmin.do','${list.id}','800','500')"
                               class="ml-5"
            ...
            <div>
                <div style="float: left">显示 ${(adminList.pageNum - 1) * adminList.pageSize + 1} 到 ${adminList.size}
                    ，共 ${adminList.total} 条
                </div>
                <nav aria-label="Page navigation" style="float: right">
                    <ul class="pagination">
                        <c:if test="${!adminList.isFirstPage}">
                            <li>
                        ...
                        <c:forEach begin="1" end="${adminList.pages}" var="pageNum">
                        ...
                        <c:if test="${!adminList.isLastPage}">
                            <li>
...
<script type="text/javascript"
        src="${pageContext.request.contextPath}/web/lib/datatables/1.10.0/jquery.dataTables.min.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/web/lib/laypage/1.2/laypage.js"></script>
<script type="text/javascript">
...
//pageInfo.pageNum是当前页面的页码
            currentPage:${adminList.pageNum},
//pageInfo.pages是总页数
            totalPages:${adminList.pages},
...
//current：当前页码
    ...
    */
    ...
    function admin_stop(obj, id) {
        layer.confirm('确认要启用或停用吗？', function (index) {
            //此处请求后台程序，下方是成功后的前台处理……
            $.ajax({
                type: "POST",
                url: "${pageContext.request.contextPath}/admin/doStatus.do",
                data: {
                    id: id
                },
                success: function (data) {
