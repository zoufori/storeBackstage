    ...
    <div class="Hui-article">
        <article class="cl pd-20">
            <div class="text-c"> 日期范围：
                ...
                <form action="${pageContext.request.contextPath}/adminRole/findByRole.do" method="post">
                    <input type="text" class="input-text" style="width:250px" placeholder="输入管理员角色名称" id="" name="text">
                    <button type="submit" class="btn btn-success" id="" name=""><i class="Hui-iconfont">&#xe665;</i> 搜角色
                    </button>
                </form>
            </div>
            <div class="cl pd-5 bg-1 bk-gray mt-20">
                <span class="l"> <a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i
                        class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;"
                                                                      onclick="admin_add('添加管理员','${pageContext.request.contextPath}/adminRole/addAdminRole.do','800','500')"
                                                                      class="btn btn-primary radius"><i
                        class="Hui-iconfont">&#xe600;</i> 添加管理员</a> </span>
                <span class="r">共有数据：<strong>${roleList.total}</strong> 条</span>
            </div>
            <table class="table table-border table-bordered table-bg">
                ...
                <tbody>
                <c:forEach items="${roleList.list}" var="list">
                    <tr class="text-c">
                        <td>${list.id}</td>
                        <td>${list.role}</td>
                        <td class="td-manage"><a title="编辑" href="javascript:;"
                                                 onclick="admin_edit('管理员角色编辑','${pageContext.request.contextPath}/adminRole/addAdminRole.do','${list.id}','800','500')"
            ...
            <div style="float: left">显示 ${(roleList.pageNum - 1) * roleList.pageSize + 1} 到 ${roleList.size}
                ，共 ${roleList.total} 条
            </div>
            <nav aria-label="Page navigation" style="float: right">
                <ul class="pagination">
                    <c:if test="${!roleList.isFirstPage}">
                        <li>
...
//pageInfo.pageNum是当前页面的页码
            currentPage:${roleList.pageNum},
//pageInfo.pages是总页数
            totalPages:${roleList.pages},
    ...
    */
    ...
    function admin_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            ...
            $.ajax({
                type: "POST",
                url: "${pageContext.request.contextPath}/adminRole/deleteAdminRole.do",
                data: {
                    "id": id
                },
